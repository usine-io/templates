...
# Optional extra request headers (all unset by default → no behaviour change):
#  - NOCODB_HOST_HEADER : sets the Host header. Lets on-host tooling hit the local
#    reverse-proxy directly and bypass an edge auth layer (e.g. Cloudflare Access):
#       NOCODB_URL=http://127.0.0.1:18080  NOCODB_HOST_HEADER=acme-db.acme.example
#    No token needed — the traffic never reaches the edge.
#  - CF_ACCESS_CLIENT_ID + CF_ACCESS_CLIENT_SECRET : Cloudflare Access service-token,
#    to reach the PUBLIC URL through an Access wall (off-host / audit). Secrets never printed.
NC_HEADERS=()
[[ -n "${NOCODB_HOST_HEADER:-}" ]] && NC_HEADERS+=(-H "Host: ${NOCODB_HOST_HEADER}")
if [[ -n "${CF_ACCESS_CLIENT_ID:-}" && -n "${CF_ACCESS_CLIENT_SECRET:-}" ]]; then
    NC_HEADERS+=(-H "CF-Access-Client-Id: ${CF_ACCESS_CLIENT_ID}" -H "CF-Access-Client-Secret: ${CF_ACCESS_CLIENT_SECRET}")
fi
...
# HTTP helpers
###############################################################################
_get()    { curl -sS -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" "$NC_URL/api/v3/$1"; }
_post()   { curl -sS -X POST -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" -H "Content-Type: application/json" "$NC_URL/api/v3/$1" -d "${2:-}"; }
_patch()  { curl -sS -X PATCH -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" -H "Content-Type: application/json" "$NC_URL/api/v3/$1" -d "${2:-}"; }
_put()    { curl -sS -X PUT -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" -H "Content-Type: application/json" "$NC_URL/api/v3/$1" -d "${2:-}"; }
_delete() { curl -sS -X DELETE -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" -H "Content-Type: application/json" "$NC_URL/api/v3/$1" ${2:+-d "$2"}; }
_upload() { curl -sS -X POST -H "xc-token: $NC_TOKEN" "${NC_HEADERS[@]+"${NC_HEADERS[@]}"}" -F "file=@$2" "$NC_URL/api/v3/$1"; }
...
ENVIRONMENT
  ...
  NOCODB_HOST_HEADER      Host header override (optional). Hit a local reverse-proxy
                          directly to bypass an edge auth layer (no token needed):
                          NOCODB_URL=http://127.0.0.1:18080 NOCODB_HOST_HEADER=acme-db.acme.example
  CF_ACCESS_CLIENT_ID     Cloudflare Access service-token id (optional) — set both
  CF_ACCESS_CLIENT_SECRET Cloudflare Access service-token secret (optional)   to reach
                          the PUBLIC URL through an Access wall (off-host / audit). Sent as
